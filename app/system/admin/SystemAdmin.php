...
class SystemAdmin extends \app\base\controller\BaseController {
    ...
    private function checkPurview() {
        ...
        if (!in_array(APP_NAME . '.' . MODULE_NAME . '.' . ACTION_NAME, (array)$this->userInfo['purview'])) {
            if(!isAjax()) {
    ...
    protected function systemDisplay($tpl = '', $autoDir = true) {
        $module = $this->infoModule['info']['url'];
        ...
        $this->assign('sysBreadCrumb', $sysMenu['breadCrumb']);
        $this->assign('sysNavSub', $sysMenu['navSub']);
