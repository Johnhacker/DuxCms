...
class SystemUserModel extends SystemModel {
    ...
    public function setLogin($userName, $passWord) {
        ...
        \Dux::session()->set('system_user', $loginInfo);
        \Dux::session()->set('system_user_sign', data_auth_sign($loginInfo));
        return true;
    }
    ...
    public function getLogin() {
        //获取登陆信息
        $loginInfo = \Dux::session()->get('system_user');
        if(empty($loginInfo)) {
            return false;
        }
        if(\Dux::session()->get('system_user_sign') <> data_auth_sign($loginInfo)) {
            return false;
        }
